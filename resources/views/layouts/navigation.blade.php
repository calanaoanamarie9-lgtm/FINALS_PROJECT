    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (auth()->user()?->isAdmin())
                        ...
                        <x-nav-link :href="route('admin.staff-applications.index')" :active="request()->routeIs('admin.staff-applications.*')">
                            {{ __('Staff apps') }}
                        </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @if (auth()->user()?->isStaff())
                    <div class="relative me-3" x-data="{ count: 0 }" x-init="
                        let fetchCount = () => fetch('{{ route('staff.notifications.unread-count') }}')
                            .then(r => r.json())
                    ...
                    ">
                        <a href="{{ route('staff.notifications.index') }}" class="relative inline-flex items-center p-1 text-gray-500 hover:text-gray-700">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                            </svg>
                            <span x-show="count > 0" style="display: none" class="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold text-white bg-red-500 rounded-full min-w-[1.25rem]" x-text="count"></span>
                        </a>
                    </div>
                @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (auth()->user()?->isStaff())
                ...
                <x-responsive-nav-link :href="route('staff.notifications.index')" :active="request()->routeIs('staff.notifications.*')">
                    {{ __('Notifications') }}
                </x-responsive-nav-link>
            @endif
            ...
            @if (auth()->user()?->isAdmin())
                ...
                <x-responsive-nav-link :href="route('admin.staff-applications.index')" :active="request()->routeIs('admin.staff-applications.*')">
                    {{ __('Staff apps') }}
                </x-responsive-nav-link>
